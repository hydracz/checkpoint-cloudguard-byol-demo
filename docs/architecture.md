...
| 区域 | 网络 | 资源 |
| --- | --- | --- |
| Primary EU（默认 West Europe） | Hub `10.60.0.0/16` | Check Point frontend/backend、日志收集 VM、Log Analytics、GRS Storage |
...
Check Point `eth0` 为 External，`eth1` 为 Internal。脚本在 Gaia 中把两个
Spoke 和日志收集子网的静态路由指向后端子网的 Azure 虚拟网关，并把 `eth1`
Anti-Spoofing topology 设为 API 枚举值 `network defined by routing`。
Azure UDR 把工作负载流量送入网络虚拟设备（NVA）；Gaia 静态路由让检查后的
流量从 `eth1` 返回。

## Access Control 与 HTTPS Inspection

Management API 脚本执行以下配置：
...
- 生成演示 Outbound CA，创建出站 HTTPS layer 和 `Inspect` 规则，然后安装 policy。

CA 私钥保留在 Check Point Management。脚本只提取公钥，并把公钥安装到两台工作负载 VM。
...
3. 脚本写入 bootstrap syslog，等待 `Syslog` 表可查询，再通过第二阶段 Terraform plan 创建 Continuous Data Export。
...
- 单实例 → Check Point High Availability 或 VMSS。
