...
保持默认 `CHECKPOINT_TRANSPORT=auto`。它会在私钥和 `management_cidr` 允许时优先
使用受限 SSH；不要仅为了 custom image 强制选择 `run-command`。Gaia FTW 会在首次
启动中重启，Azure Run Command extension 如果恰好跨越该窗口，可能无法完成安装。
