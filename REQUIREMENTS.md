...
6. **交付形式**：`deploy.sh` 默认只创建基础设施；日志导出、策略自动化和测试均为独立显式步骤。
7. **管理平面**：Check Point 使用独立私网 management NIC；同 subnet 部署无公网 IP 的 Windows 管理工作站，并通过 Azure Bastion 登录，供后续安装 SmartConsole。
...
| R3 | 启用 TLS 时，SmartConsole 手工配置或可选内网脚本创建 Outbound CA 和 `Inspect` 规则；两台工作负载 VM 只安装公钥 CA。 |
...
| R9 | `eth0 10.60.3.4` 是只绑定私网管理 NSG 的 primary NIC，`eth1 10.60.0.4` 是绑定 Public IP 的 frontend，`eth2 10.60.1.4` 是 backend/NVA next hop；数据平面 NSG 不允许公网来源访问管理端口。 |
| R10 | Windows `10.60.3.10` 与 Gateway `eth0` 共享 management subnet、没有 Public IP，TCP/3389 只接受 `AzureBastionSubnet`；默认部署不运行 Check Point 配置或自动测试。 |
...
- Gaia `admin` 密码。额外管理来源必须是可路由到 management subnet 的可信私网/VPN，
  不接受 `0.0.0.0/0`。
